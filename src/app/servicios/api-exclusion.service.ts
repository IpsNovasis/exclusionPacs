...
export class ApiExclusionService {
  ...
  private urlExcdel: string = environment.urlIncIns;
  ...
  // Elimina registro  en tabla tblPACSExclusionConcesion e inserta movimiento en tabla tblBNFMovimientos
  InsDelInclusion(exclusion: Exclusion): Observable<Exclusion> {
    return this.http.post<Exclusion>(this.urlExcdel, exclusion, { headers: this.httpHeaders }).pipe(
      map((response: any) => response.exclusion as Exclusion),
      catchError(e => {
        if (e.status == 400) {
          console.error(e.error.mensaje, 'code : 400, InsDelInclusion');
          return throwError(e);
        }
        console.error(e.error.mensaje);
        return throwError(e);
